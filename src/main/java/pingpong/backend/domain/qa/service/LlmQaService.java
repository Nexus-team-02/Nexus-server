...
public class LlmQaService {
  ...
  [NEGATIVE 케이스 데이터 규칙 - 필수 준수]
     - [404 Not Found]: '존재하지 않는 ID(999999)'를 조회하거나 조작하는 시나리오에 할당하세요.
     - [400 Bad Request]: 파라미터 타입 오류(숫자 자리에 "abc" 입력) 또는 필수 파라미터 누락 시 할당하세요.
     - [422 Unprocessable Entity]: 비즈니스 로직 위반(예: 제목은 100자 이내인데 200자 입력, 유효하지 않은 날짜 형식 등) 시 할당하세요.
     - [409 Conflict]: 중복된 데이터 등록(예: 이미 존재하는 아이디로 가입 시도) 시 할당하세요.
   """;
